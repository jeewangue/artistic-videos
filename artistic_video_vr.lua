...
require 'image'
require 'loadcaffe'
require 'artistic_video_core_vr'
...
cmd:option('-flow_pattern_spatial_lr','flow_spatial/frame/',
'Optical flow for spatial files pattern')
cmd:option('-flow_pattern_spatial_rl','flow_spatial/frame/',
'Optical flow for spatial files pattern')
cmd:option('-flowWeight_pattern', 'example/deepflow/reliable_[%d]_{%d}.pgm',
'Optical flow weight files pattern.')
cmd:option('-flowWeight_pattern_spatial_lr','flow_spatial/frame/',
'Optical flow weight files pattern.')
cmd:option('-flowWeight_pattern_spatial_rl','flow_spatial/frame/',
'Optical flow weight files pattern.')
...
local function main(params)
	...
	for frameIdx=params.start_number + params.continue_with - 1, params.start_number + num_images - 1 do
		...
		local content_losses, temporal_losses, spatial_losses = {}, {}, {}
		...
		print(losses_indices)
		print(losses_type)

		-- Add content , temporal and spatial loss for this iteration. Style loss is already included in the net.
		for i=1, #losses_indices do
			if losses_type[i] == 'content'  then
				...
				print("content_loss_module is inserted at ", losses_indices[i] ,"+" , additional_layers)
			...
			elseif losses_type[i] == 'prevPlusFlowWeighted' and frameIdx > params.start_number then
				...
				-- Add spatial loss module

			elseif losses_type[i] == 'spatial' then
				local image_spatial= nil
				-- target initailization : content_image or prevWarped by temporal optical flow
				if frameIdx == params.start_number then
					image_spatial = content_image

				elseif frameIdx > params.start_number then
					local fileName_spatial = build_OutFilename(params, math.abs(frameIdx - params.start_number + 1), -1)
					image_spatial = image.load(fileName_spatial,3)
				end
				-- target initialization. content_image or prevWarped by temporal optical flow

				local weightsFileName_lr = getFormatedFlowFileNameSpatial(params.flowWeight_pattern_spatial_lr, math.abs(frameIdx))
				local weightsFileName_rl = getFormatedFlowFileNameSpatial(params.flowWeight_pattern_spatial_rl, math.abs(frameIdx))

				print(string.format('Reading flowWeights files "%s", "%s".', weightsFileName_lr, weightsFileName_rl))

				-- Preprocess flow weights, calculate long-term weights
				-- ** delete this part. do not need unless using inverse option processFlowWeights(flowWeightsTabl, params.combine_flowWeights_method, params.invert_flowWeights)
				-- Create loss modules, one for each previous frame warped

				flowWeights_lr = image.load(weightsFileName_lr):float()
				flowWeights_rl = image.load(weightsFileName_rl):float()

				--QQ  merge to flowWeights. 하나로.


				local flowFileNameSpatial_lr = getFormatedFlowFileNameSpatial(params.flow_pattern_spatial_lr, math.abs(frameIdx))
				local flowFileNameSpatial_rl = getFormatedFlowFileNameSpatial(params.flow_pattern_spatial_rl, math.abs(frameIdx))

				local flow_spatial_lr = flowFile.load(flowFileNameSpatial_lr)
				local flow_spatial_rl = flowFile.load(flowFileNameSpatial_rl)

				-- flow_spatial_lr 과 rl을 합쳐서 넘기기.


				--flowWeights = flowWeights:expand(3, flowWeights:size(2), flowWeights:size(3))
				--flowWeights = MaybePutOnGPU(flowWeights, params)

				local loss_module = getSpatialLossModuleForLayer(net,
				losses_indices[i] + additional_layers, image_spatial, flow_spatial, warpImage, flowWeights, params)
				net:insert(loss_module, losses_indices[i] + additional_layers)
				table.insert(spatial_losses, loss_module)
				print("spatial loss module is inserted at ", losses_indices[i], "+", additional_layers)  
				additional_layers = additional_layers + 1
		...
		runOptimization(params, net, content_losses, style_losses, temporal_losses, spatial_losses, img, frameIdx, -1, num_iterations)
		...
		for i=#losses_indices, 1, -1 do
			if frameIdx > params.start_number or losses_type[i] == 'content' or losses_type[i] == 'spatial' then
